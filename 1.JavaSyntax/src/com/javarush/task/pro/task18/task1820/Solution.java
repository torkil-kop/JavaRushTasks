...
public class Solution {
    ...
    public static Optional<Car> getMostExpensiveCar(Stream<Car> cars) {
        Optional<Car> carOptional = cars.max((i1, i2) -> i1.getPrice() - i2.getPrice());

       //Car ii = cars.max((i1, i2) -> i1.getPrice() - i2.getPrice()).get();
        return carOptional;
    }

    public static Optional<Car> getMoreExpensiveCar(Stream<Car> cars, Car mostExpensiveCar) {
        Optional<Car> carOptional2 = cars.filter(i -> i.getPrice() > mostExpensiveCar.getPrice()).findFirst();
